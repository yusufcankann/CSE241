...
/*Copy constructer.*/
template<class E,class C>
HashSet<E,C>::HashSet(const HashSet &c){
...
/*Assignment operator.*/
template<class E,class C>
HashSet<E,C>& HashSet<E,C>::operator=(HashSet &c){
	/*First it controls if elements are the same or what?*/
...
template<class E,class C>
Iterator<E,C> HashSet<E,C>::iterator()throw(){
	Iterator<E,C> iter(*inner);
	return iter;
}

/*
 * It adds the element. If element
 * is in the set it throw exception.
 */
template<class E,class C >
void HashSet<E,C>::add(E e)throw(Duplicate){
...
template<class E,class C>
void HashSet<E,C>::addAll(Collection<E,C> &c)throw(){
...
template<class E,class C >
void HashSet<E,C>::clear()throw(){
	(*inner).clear();
}

/*Checks every element if it contains or not.*/
template<class E,class C>
bool HashSet<E,C>::contains(E e)throw(){
	for(auto a:(*inner)){
		if(a==e) return 1;
	}
	return 0;
...
template<class E,class C >
bool HashSet<E,C>::containsAll(Collection<E,C> &c)throw(){
	bool a=0;
	Iterator<E,C> iter = c.iterator();
...
template<class E,class C >
bool HashSet<E,C>::isEmpty()throw(){
	return ((*inner).empty());
}

template<class E,class C >
void HashSet<E,C>::remove(E e)throw(ZeroSize){
...
template<class E,class C >
void HashSet<E,C>::removeAll(Collection<E,C> &c)throw(){
	Iterator<E,C> iter  = c.iterator();
	while (iter.hasNext()){
...
template<class E,class C >
void HashSet<E,C>::retainAll(Collection<E,C> &c)throw(){
	bool a=0;
	bool mark=0;
...
template<class E,class C >
int HashSet<E,C>::size()throw(){
	return (*inner).size();
}

template<class E,class C>
void HashSet<E,C>::printElements()throw(){
	for(auto i : (*inner)){
		cout<<" "<<i<<" ";
	}
}
